...
app.post("/register", registerRules, sanitizeBody, validateBody, async (req, res) => {
    ...
    if (req.cookies.accessToken || req.cookies.refreshToken) return res.status(403).json({ message: "Te encuentras logueado!" });
...
app.post("/login", loginRules, sanitizeBody, validateBody, async (req, res) => {
    ...
    const refreshToken = jwt.sign(
        ...
        { expiresIn: process.env.JWT_REFRESH_EXPIRES_IN } //*Expiracion del token
    );
    ...
    res.cookie("refreshToken", refreshToken, {
        ...
        maxAge: 7 * 24 * 60 * 60 * 1000 //*Vencimiento de la cookie (duración de sesión = 7d)
    });
...
app.post("/refresh", (req, res) => {
    ...
    if (!refreshToken) {
        return res.status(401).json({ message: "RefreshToken requerido" });
    }
    ...
    try {
        ...
        const newAccessToken = jwt.sign(
            { id: decoded.id, dni: decoded.dni, role: decoded.role },
            process.env.SECRET_KEY_JWT,
